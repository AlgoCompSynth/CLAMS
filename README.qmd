...
format: gfm
editor: visual
bibliography: CLAMS-Quarto-Site/CLAMS.bib
---
...
-   The ability to build in C/C++ libraries at link time and execute them as Forth words.
    ...
    Given the volatile current state of RP2040 audio hardware, there aren't really any device-specific libraries I can use aside from the [Arduino ecosystem](https://github.com/pschatzmann/arduino-audio-tools "Arduino Audio Tools on GitHub") and MicroPython / CircuitPython. I'll have to go straight to the hardware anyhow, and `zeptoforth` has all the low-level words to do that already.

    So I will be building `CLAMS` on `zeptoforth`. I'm planning to write enough documentation so that non-Forth programmers can follow along. So, as the saying goes, "Watch this space."
